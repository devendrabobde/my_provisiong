class OldPassword < ActiveRecord::Base
  ...
  set_primary_key :sys_old_password_id

  alias_attribute :sys_old_password_id, :id
end
    ...
    module PasswordArchivable
      ...
      def password_archive_included?
        ...
        if self.class.deny_old_passwords > 0 and not self.password.nil?
          old_passwords_including_cur_change = self.old_passwords.order(:sys_old_password_id).reverse_order.limit(self.class.deny_old_passwords)
      ...
      def archive_password
        if self.encrypted_password_changed?
          if self.class.password_archiving_count.to_i > 0
            ...
            self.old_passwords.order(:sys_old_password_id).reverse_order.offset(self.class.password_archiving_count).destroy_all
          else
