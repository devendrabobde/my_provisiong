Feature: COA upload providers and EPCS application is unavailable/unreachable (network interruption) after upload begin. In this case provisioing system should be showing connection error message along with error code on UI (provider detail page) corresponding to each provider.
...
Background:
  ...
  @selenium
  @no-database-cleaner
  Scenario: COA upload providers and EPCS application is unavailable after upload begin
    ...
    And I should be able to see EPCS acknowledgement or error messages

  @selenium
  @no-database-cleaner
  Scenario: COA upload providers and EPCS application is unreachable (network interruption) after upload begin
    Given I select an application
    When I select a csv file of 4 providers
    And I clicks upload button
    And I should be able to see correct file upload message
