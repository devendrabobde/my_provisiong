...
When /^I select a csv file which is containing one single batch of multiple providers, some pass WsBatchIdp, while the others fail WsBatchIdp$/ do
  attach_file 'upload', File.join(Rails.root, 'public', 'rspec_test_files', 'epcs', 'batch_of_multiple_providers_some_pass_wsbatchidp_some_are_fail.csv.csv') # id
end
...
When /^I select a csv file which is containing more than one batch of providers from the same spreadsheet, some pass WsBatchIdp, while the others fail WsBatchIdp$/ do
  attach_file 'upload', File.join(Rails.root, 'public', 'rspec_test_files', 'epcs', 'more_than_one_batch_of_multiple_providers_some_pass_wsbatchidp_some_are_fail.csv.csv') # id
end