...
describe "ProvisioingCsvValidation" do
  ...
  describe "#process_csv_api" do
      ...
      app_hash_router = @router_reg_apps.collect{|x| x.values.flatten.select{|y| y if "#{x.keys.first}::#{y['ois_name']}" == application.display_name}}.flatten.first rescue nil
  	  response = ProvisioingCsvValidation.process_csv_api(path, application, app_hash_router)  		
  ...
  describe "#validate_provider_api" do
    ...
    it "should process the csv file of providers for EPCS-IDP application" do
      ...
      application = RegisteredApp.where(app_name: CONSTANT["APP_NAME"]["EPCS"]).first
      app_hash_router = @router_reg_apps.collect{|x| x.values.flatten.select{|y| y if "#{x.keys.first}::#{y['ois_name']}" == application.display_name}}.flatten.first rescue nil
      response = ProvisioingCsvValidation.validate_provider_api(@providers, application, app_hash_router)      
      assert response.should be_true
    end
